document.addEventListener('DOMContentLoaded', () => {
    ...
    function createConfetti() {
        const colors = ['#6366f1', '#a855f7', '#ec4899', '#10b981', '#f59e0b', '#06b6d4', '#ffffff'];
    ...
    // === Кнопка "Отпраздновать" ===
    const celebrateBtn = document.getElementById('celebrateBtn');
    if (celebrateBtn) {
        celebrateBtn.addEventListener('click', () => {
            createConfetti();
            
            // Дополнительный эффект - мигание
            const body = document.body;
            body.style.transition = 'background 0.3s ease';
            body.style.background = 'radial-gradient(ellipse at 20% 30%, #1a1a3e, #0a0a2a)';
            setTimeout(() => {
    ...
    if (restartBtn) {
        restartBtn.addEventListener('click', () => {
            // Отправляем сообщение родительскому окну для перехода на первый слайд
            window.parent.postMessage({ type: 'goToSlide', slide: 1 }, '*');
        });
    }
    
    // === Анимация карточек при клике ===
    ...
    summaryCards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            showToast(`✨ ${title} — ключевая технология современной разработки`);
        });
    });
    ...
    function showToast(message) {
        ...
        toast.style.background = 'linear-gradient(135deg, rgba(99, 102, 241, 0.95), rgba(168, 85, 247, 0.95))';
    ...
    const sections = document.querySelectorAll('.summary-cards, .key-takeaways, .quote-section, .thankyou-section');
            ...
            if (entry.isIntersecting) {
                ...
                if (valueEl) {
                    ...
                    if (match) {
                        ...
                        const suffix = text.replace(target.toString(), '');
                        animateCounter(valueEl, target, suffix, 1500);
                    }
                }
    ...
    const statItems = document.querySelectorAll('.summary-card .card-stats');
    statItems.forEach(item => {
        statObserver.observe(item);
    });
    
    console.log('🎉 Страница 10 "Заключение" загружена! Презентация завершена! 🎉');
});